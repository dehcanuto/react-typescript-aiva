...
jest.mock('next/link', () => {
  const MockLink = ({ href, children }: { href: HrefType; children: React.ReactNode }) => (
...
describe('ProductCard', () => {
  it('renderiza dados do produto corretamente', () => {
    ...
    expect(img).toHaveAttribute('src', '/_next/image?url=https%3A%2F%2Fi.imgur.com%2FQphac99.jpeg&w=640&q=75');
    expect(screen.getByTestId('favorite-button')).toBeInTheDocument();

    const productLinks = screen.getAllByRole('link', { name: "Sleek Modern Leather Sofa" });
    ...
    expect(categoryLink).toHaveAttribute('href', `/category/${product.category.id}`);
  });

  it('renderiza imagem padrão quando não há imagem', () => {
    render(<ProductCard {...{ ...product, images: [] }} />);
    expect(screen.getByAltText(product.title)).toHaveAttribute('src', '/_next/image?url=%2Fno-image.png&w=640&q=75');
  });
});
